...
version of this example, and ran it on a 40 core Intel Xeon \alert{XX} with \alert{XX}
sockets. The solution at the end of the \alert{XX} time steps is shown on the
left.  The efficiency for selected numbers of cores using strong
scaling is shown in the right plot.
\alert{SHOW PERCENTAGE OF TIME IN STEPGRID VERSUS OTHER STUFF (TABLE?)}.
