...
\revised{Unlike, for example, tree-based methods that refine on a cell by cell basis
\amrclaw flags cells needing refinement and collects them into rectangular grid
patches.  Finer grids must be nested within the union of grids at the next
coarser level.}
...
as an indicator of work.  In addition there is a grids are limited to a 
maximum of 32 cells in each dimension, otherwise they are bisected until this
condition is met.}  Note that this approach causes a memory bulge. Each
